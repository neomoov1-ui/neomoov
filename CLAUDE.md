...
## Design
Les maquettes et les règles d'interface sont dans `docs/design/` : identité (couleurs, typographie, composants) dans `02-prompts-claude-design.md` (prompt 0), sources des écrans dans `canevas/project/`, analyse de la référence et modèle des relevés dans `01-analyse-reference-uber.md`. Les écrans codés suivent ces maquettes. Les écarts E1 à E12 de l'analyse ne sont pas codés tant que le fondateur ne les a pas arbitrés.
