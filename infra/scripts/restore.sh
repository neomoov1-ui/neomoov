...
docker run --rm -e TARGET_DATABASE_URL "$PG_IMAGE" sh -c 'psql "$TARGET_DATABASE_URL" -v ON_ERROR_STOP=1 -c "CREATE EXTENSION IF NOT EXISTS postgis; CREATE EXTENSION IF NOT EXISTS pgcrypto;"'

echo "Restauration dans la cible"
openssl enc -d -aes-256-cbc -pbkdf2 -iter 200000 -pass env:BACKUP_PASSPHRASE -in "$FILE" \
  | docker run --rm -i -e TARGET_DATABASE_URL "$PG_IMAGE" sh -c 'pg_restore --clean --if-exists --no-owner --no-privileges --exit-on-error -d "$TARGET_DATABASE_URL"'

echo "Contrôles après restauration"
docker run --rm -e TARGET_DATABASE_URL "$PG_IMAGE" sh -c 'psql "$TARGET_DATABASE_URL" -At -v ON_ERROR_STOP=1 -c "
