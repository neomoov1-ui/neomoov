...
| `BOOKING_FRAME_ANCESTORS` (variable, facultative) | Sites autorisés à intégrer `/reserver` en iframe, séparés par des espaces | Variables du dépôt (images GitHub) ; `/opt/neomoov/.env` (construction sur le serveur) | Vide : seuls `https://neomoov.net` et `https://www.neomoov.net` peuvent intégrer la réservation ; à remplir pour un site partenaire. Figée au build : reconstruire l'image du web après changement | Facultatif |
