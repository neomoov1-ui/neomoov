...
describe('adaptateurs simulés', () => {
  ...
  it('un adaptateur réel est un objet ordinaire pour NestJS et le journal (pas de crochets fantômes)', () => {
    ...
    expect(inspect(realStorage(env), { depth: 5, showHidden: true })).not.toContain('s3_secret_x');
    expect(JSON.stringify(realStorage(env))).not.toContain('s3_secret_x');
    expect(() => realStorage(loadEnv({ NODE_ENV: 'test', DATABASE_URL: 'postgresql://x', S3_ACCESS_KEY: 'k' }, { dotenv: false }))).toThrow(/S3_ENDPOINT/);
  });
});
