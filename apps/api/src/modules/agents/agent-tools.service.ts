  ...
  private async escalateToHuman(ctx: AgentRunContext, input: z.infer<typeof escalateToHumanToolSchema>): Promise<ToolResult> {
    ...
    if (input.reason === 'safety') {
      const [incident] = await this.db
        .insert(schema.incidents)
        .values({ type: 'complaint', severity: 'high', reportedByUserId: ctx.subjectUserId, reportedByKind: 'agent', description: `Plainte de sécurité transmise par l'agent : ${summary}`.slice(0, 2_000) })
        .returning({ id: schema.incidents.id });
      await this.audit.recordSystem({ action: 'agent.incident_opened', entity: 'incidents', entityId: incident!.id, after: { type: 'complaint', severity: 'high', reason: 'safety' } }, ctx.agent.code);
    }
